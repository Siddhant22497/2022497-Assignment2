...
void* handle_client(void* soc_des) {
    ...
    snprintf(response, sizeof(response),
             ...
             "1. PID: %d, Name: %s, CPU  User Time: %lld, CPU Kernel Time: %lld, CPU Total Time (User + Kernel): %lld\n"
             "2. PID: %d, Name: %s, CPU  User Time: %lld, CPU Kernel Time: %lld, CPU Total Time (User + Kernel): %lld\n",
...
int main() {
    ...
    while ((new_socket = accept(server_fd, (struct sockaddr*)&address, (socklen_t*)&addrlen)) >= 0) {
        ...
        *new_sock = new_socket;
        ...
        // giving each client socket a new thread
